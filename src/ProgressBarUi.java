...
public class ProgressBarUi extends BasicProgressBarUI {
    ...
    public ProgressBarUi() {
        try {
            bimage = ImageIO.read(this.getClass().getResource("/brick_load.png"));
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
    ...
    private void paintString(Graphics g, int x, int y, int w, int h, int fillStart, int amountFull) {
        ...
        if (progressBar.getOrientation() == SwingConstants.HORIZONTAL) {
            ...
            BasicGraphicsUtils.drawString( g2, progressString,amountFull ,renderLocation.x, renderLocation.y);
